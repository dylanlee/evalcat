...
// Top-level pathing
InsOuts: {
	eval_out: string & "s3://fimc-data/hand_fim/"
	hand_in:  string & "s3://noaa-nws-owp-fim/hand_fim/inputs/"
	hand_out: string & "s3://noaa-nws-owp-fim/hand_fim/"
}
...
#HandRem: {
	...
	huc:    string & =~"^[0-9]+$"
	branch: string & =~"^[0-9]+$"
	dir_path: path.Join([InsOuts.hand_out, data_version, huc, "branches", branch], path.Unix)
}
...
#Huc8Shape: {
	...
	huc: string & =~"^[0-9]+$"
	dir_path: path.Join([InsOuts.hand_out, data_version, huc], path.Unix)
}
...
#HucBranchMap: {
	...
	dir_path: path.Join([InsOuts.hand_out, data_version], path.Unix)
}
...
#EvalMetrics: {
	...
	dir_path: path.Join([InsOuts.eval_out, "metrics"], path.Unix)
}
